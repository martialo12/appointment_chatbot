...
#  description        : This library simplifies the JSON response building in Python for
#                       Dialogflow fulfillment. Using it you can build the JSON objects with few line.
...
class actions_on_google_response:
    ...
    def simple_response(self, responses):
        ...
        if len(responses) > 2:
            ...
            return {
                ...
                "simpleResponses": {"simpleResponses": responses_json},
            }

    """"
    Actions on Google Card Builder
    @param title = string
    ...
    def card(self, title, subtitle="", image=None, buttons=None):
        ...
        logger.debug(f"response card: {response}")
        return response
    ...
    def basic_card(
        self, title, subtitle="", formattedText="", image=None, buttons=None
    ):
        # list to store buttons responses
        buttons_json = []
        if buttons is not None:
            ...
            response = {
                ...
                "basicCard": {
                    ...
                    "buttons": buttons_json,
                    "image": {"imageUri": image[0], "accessibilityText": image[1]},
                },
            }
    ...
    def list_select(self, list_title, list_elements):
                ...
                items_list.append(
                    {
                        ...
                        "info": {
                            ...
                            # synonyms are the words that can be used as a value for the option
                            # when the user types instead of selecting from the list
        ...
        return {
            "platform": self.platform,
            "listSelect": {"title": list_title, "items": items_list},
        }
    ...
    def suggestion_chips(self, suggestions):
        ...
        return {
            "platform": self.platform,
            "suggestions": {"suggestions": suggestions_json},
        }
    ...
    def link_out_suggestion(self, title, url):
        ...
        if title == "" or url == "":
            ...
            return {
                "platform": self.platform,
                "linkOutSuggestion": {"destinationName": str(title), "uri": str(url)},
            }
...
class facebook_response:
    ...
    def text_response(self, texts):
        ...
        if len(texts) <= 0:
            ...
            return {"text": {"text": text_obj}, "platform": self.platform}
    ...
    def quick_replies(self, title, quick_replies_list):
        ...
        # quick_replies_list must contains at least one string
            ...
            for quick_reply in quick_replies_list:
                # append to the list
                quick_replies.append(str(quick_reply))

            # return the response JSON
            return {
                "quickReplies": {"title": str(title), "quickReplies": quick_replies},
                "platform": self.platform,
            }
    ...
    def image_response(self, url):
        ...
        if url == "":
            ...
            return {"image": {"imageUri": str(url)}, "platform": self.platform}

    def card_response(self, title, buttons):
        buttons_json = []
        for button in buttons:
            buttons_json.append({"text": str(button[0]), "postback": str(button[1])})

        # return the card
        return {
            "card": {"title": str(title), "buttons": buttons_json},
            "platform": self.platform,
        }
    ...
    def custom_payload(self, payload):
        ...
        return {"payload": payload, "platform": self.platform}
...
class fulfillment_response:
    ...
    def fulfillment_text(self, fulfillmentText):
        if fulfillmentText == "":
            ...
            return {"fulfillment_text": str(fulfillmentText)}
    ...
    def fulfillment_messages(self, response_objects):
        if len(response_objects) <= 0:
            ...
            return {"fulfillment_messages": response_objects}
    ...
    def output_contexts(self, session, contexts):
        ...
        return {"output_contexts": contexts_json}
    ...
    def followup_event_input(self, name, parameters):
        return {"followup_event_input": {"name": str(name), "parameters": parameters}}
            ...
            if output_contexts is not None:
                if fulfillment_messages is not None:
                    response = {
                        ...
                        "followupEventInput": followup_event_input[
